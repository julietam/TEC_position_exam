#!/usr/bin/env python3
"""
Comprehensive Data Acquisition and Preprocessing for CO2 Emissions Analysis
...
class WorldBankDataProcessor:
    ...
    def __init__(self, start_year=1990, end_year=2022):
        ...
        self.indicators = {
            ...
            'CC.CO2.EMSE.EL': 'CO2_emissions_Mt',  # Total CO2 emissions excluding LUCF (Mt CO2)
            'CC.CO2.EMSE.EN': 'CO2_emissions_energy_Mt',  # CO2 emissions from energy sector
            'CC.CO2.EMSE.TR': 'CO2_emissions_transport_Mt',  # CO2 emissions from transportation
        ...
        for country in data['country'].unique():
            ...
            for col in numeric_columns:
                if country_data[col].isnull().any():
                    ...
                    data.loc[country_mask & null_mask, col] = filled_values[null_mask]
    ...
    def _handle_outliers(self, data):
        ...
        for col in numeric_columns:
            ...
            if 'CO2_emissions' in col:
                ...
                print(f"Skipping outlier handling for {col} to preserve emission data")
                continue
    ...
    def _engineer_features(self, data):
        ...
        if 'CO2_emissions_Mt' in data.columns and 'Population_total' in data.columns:
            ...
            data['CO2_emissions_per_capita'] = (data['CO2_emissions_Mt'] * 1_000_000) / data['Population_total']
    ...
    def create_visualizations(self, output_dir):
        ...
        if 'CO2_emissions_per_capita' in self.processed_data.columns:
            ...
            latest_data = self.processed_data[self.processed_data['date'] == latest_year]

            # Get top CO2 emitting countries per capita
            top_countries = latest_data.nlargest(10, 'CO2_emissions_per_capita')['country'].tolist()
        ...
        if 'GDP_per_capita_current_USD' in self.processed_data.columns and 'CO2_emissions_per_capita' in self.processed_data.columns:
            ...
            print(f"Creating scatter plot with {len(latest_data)} countries for year {latest_year}")

            fig, ax = plt.subplots(figsize=(10, 8))
            
            # Filter out missing data
            plot_data = latest_data.dropna(subset=['GDP_per_capita_current_USD', 'CO2_emissions_per_capita'])
            ...
            if len(plot_data) > 0:
                ...
                print(f"Scatter plot created with {len(plot_data)} data points")
            else:
